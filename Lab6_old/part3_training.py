...
def train(model, train_data, valid_data=None, exp_dir='exp_00'):
    ...
    # my callbacks
    es_cbk = tf.keras.callbacks.EarlyStopping(monitor='loss', patience=3)
    ckp_cbk = tf.keras.callbacks.ModelCheckpoint(filepath=os.path.join(exp_dir, 'model_best_weights'), save_best_only=True)
...
def get_random_hp(constants):
    ...
    hidden_dims = []
    for _ in range(random.randint(hidden_dims_len_range[0], hidden_dims_len_range[1])):
    ...
    lr = pow(10, random.randint(lr_exponent_range[0], lr_exponent_range[1]))
