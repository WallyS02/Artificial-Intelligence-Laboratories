...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            ...
            left_gini = 1 - ((left_positive / (left_negative + left_positive)) ** 2 + (left_negative / (left_negative + left_positive)) ** 2)
            right_gini = 1 - ((right_positive / (right_negative + right_positive)) ** 2 + (right_negative / (right_negative + right_positive)) ** 2)
            gain = 1 - ((left / (left + right)) * left_gini + ((right / (left + right)) * right_gini))
            if gain > best_gain:
                best_gain = gain
                best_idx = idx
