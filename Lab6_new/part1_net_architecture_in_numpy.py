...
def zad2_two_layer_net(student_id):
    ...
    model.hidden_layer.W[:, 0] = [1.1, 0.75]    # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [1.5, 1]       # wagi neuronu h2
    model.hidden_layer.b[:] = [-1.5, -0.5]      # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [1.5, -1.5]    # wagi neuronu wyjściowego
